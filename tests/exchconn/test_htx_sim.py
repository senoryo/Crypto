"""Tests for exchconn/htx_sim.py — submit/cancel/amend order lifecycle."""
...
from exchconn.htx_sim import HTXSimulator, BASE_PRICES, PRICE_JITTER_PCT
...
@pytest.fixture
def sim():
    """Create an HTXSimulator with a mock report callback (no price jitter loop)."""
    simulator = HTXSimulator()
    simulator._report_callback = AsyncMock()
    return simulator


# -----------------------------------------------------------------------
# TestHTXOrderIdGeneration
# -----------------------------------------------------------------------

class TestHTXOrderIdGeneration:

    def test_prefix(self, sim):
        oid = sim._next_order_id()
        assert oid.startswith("HTX-")
        assert oid == "HTX-000001"

    def test_sequential(self, sim):
        id1 = sim._next_order_id()
        id2 = sim._next_order_id()
        assert id1 == "HTX-000001"
        assert id2 == "HTX-000002"


# -----------------------------------------------------------------------
# TestHTXGetCurrentPrice
# -----------------------------------------------------------------------

class TestHTXGetCurrentPrice:
