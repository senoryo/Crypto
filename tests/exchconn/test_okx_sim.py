"""Tests for exchconn/okx_sim.py — submit/cancel/amend order lifecycle."""
...
from exchconn.okx_sim import OKXSimulator, BASE_PRICES, PRICE_JITTER_PCT
...
@pytest.fixture
def sim():
    """Create an OKXSimulator with a mock report callback (no price jitter loop)."""
    simulator = OKXSimulator()
    simulator._report_callback = AsyncMock()
    return simulator


# -----------------------------------------------------------------------
# TestOKXOrderIdGeneration
# -----------------------------------------------------------------------

class TestOKXOrderIdGeneration:

    def test_prefix(self, sim):
        oid = sim._next_order_id()
        assert oid.startswith("OKX-")
        assert oid == "OKX-000001"

    def test_sequential(self, sim):
        id1 = sim._next_order_id()
        id2 = sim._next_order_id()
        assert id1 == "OKX-000001"
        assert id2 == "OKX-000002"


# -----------------------------------------------------------------------
# TestOKXGetCurrentPrice
# -----------------------------------------------------------------------

class TestOKXGetCurrentPrice:
