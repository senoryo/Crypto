...
def _make_connector():
    ...
    with patch("exchconn.exchconn.WSServer") as MockWSServer, \
        ...
        connector._exchanges = {"BINANCE": mock_binance, "COINBASE": mock_coinbase}
        connector._server = mock_server

    return connector, mock_binance, mock_coinbase, mock_server


@pytest.fixture
def connector_env():
    """Fixture returning (connector, binance_mock, coinbase_mock, server_mock)."""
...
class TestExDestinationRouting:
    ...
    async def test_rejects_unknown_exchange(self, connector_env):
        connector, binance, coinbase, server = connector_env
        msg = new_order_single("C1", "BTC/USD", Side.Buy, 1.0, OrdType.Limit, 67000.0, exchange="KRAKEN")
        raw = msg.to_json()
        ...
        assert "KRAKEN" in sent.get(Tag.Text)
...
class TestDefaultRouting:
    ...
    @pytest.mark.asyncio
    async def test_doge_defaults_to_coinbase(self, connector_env):
        connector, binance, coinbase, server = connector_env
