"""Tests for exchconn/bitfinex_sim.py — submit/cancel/amend order lifecycle."""
...
from exchconn.bitfinex_sim import BitfinexSimulator, BASE_PRICES, PRICE_JITTER_PCT
...
@pytest.fixture
def sim():
    """Create a BitfinexSimulator with a mock report callback (no price jitter loop)."""
    simulator = BitfinexSimulator()
    simulator._report_callback = AsyncMock()
    return simulator


# -----------------------------------------------------------------------
# TestBitfinexOrderIdGeneration
# -----------------------------------------------------------------------

class TestBitfinexOrderIdGeneration:

    def test_prefix(self, sim):
        oid = sim._next_order_id()
        assert oid.startswith("BFX-")
        assert oid == "BFX-000001"

    def test_sequential(self, sim):
        id1 = sim._next_order_id()
        id2 = sim._next_order_id()
        assert id1 == "BFX-000001"
        assert id2 == "BFX-000002"


# -----------------------------------------------------------------------
# TestBitfinexGetCurrentPrice
# -----------------------------------------------------------------------

class TestBitfinexGetCurrentPrice:
...
# -----------------------------------------------------------------------
# TestBitfinexSubmitOrder
# -----------------------------------------------------------------------

class TestBitfinexSubmitOrder:
...
# -----------------------------------------------------------------------
# TestBitfinexCancelOrder
# -----------------------------------------------------------------------

class TestBitfinexCancelOrder:
...
# -----------------------------------------------------------------------
# TestBitfinexAmendOrder
# -----------------------------------------------------------------------

class TestBitfinexAmendOrder:
