"""Tests for exchconn/kraken_sim.py — submit/cancel/amend order lifecycle."""
...
from exchconn.kraken_sim import KrakenSimulator, BASE_PRICES, PRICE_JITTER_PCT
...
@pytest.fixture
def sim():
    """Create a KrakenSimulator with a mock report callback (no price jitter loop)."""
    simulator = KrakenSimulator()
    simulator._report_callback = AsyncMock()
    return simulator


# -----------------------------------------------------------------------
# TestKrakenOrderIdGeneration
# -----------------------------------------------------------------------

class TestKrakenOrderIdGeneration:

    def test_prefix(self, sim):
        oid = sim._next_order_id()
        assert oid.startswith("KRK-")
        assert oid == "KRK-000001"

    def test_sequential(self, sim):
        id1 = sim._next_order_id()
        id2 = sim._next_order_id()
        assert id1 == "KRK-000001"
        assert id2 == "KRK-000002"


# -----------------------------------------------------------------------
# TestKrakenGetCurrentPrice
# -----------------------------------------------------------------------

class TestKrakenGetCurrentPrice:
...
# -----------------------------------------------------------------------
# TestKrakenSubmitOrder
# -----------------------------------------------------------------------

class TestKrakenSubmitOrder:
...
# -----------------------------------------------------------------------
# TestKrakenCancelOrder
# -----------------------------------------------------------------------

class TestKrakenCancelOrder:
...
# -----------------------------------------------------------------------
# TestKrakenAmendOrder
# -----------------------------------------------------------------------

class TestKrakenAmendOrder:
