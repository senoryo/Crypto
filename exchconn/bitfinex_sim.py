"""
Simulated Bitfinex Exchange Adapter.

Accepts orders, generates execution reports with realistic delays and partial fills.
Order IDs are prefixed with "BFX-".
Bitfinex characteristics: tightest spreads (0.07% jitter), deeper order book,
medium-slow fills (0.5-2.5s delay), wider price variation (+-0.025%).
"""
...
# Bitfinex has the tightest spreads
PRICE_JITTER_PCT = 0.0007  # 0.07%
...
class BitfinexSimulator:
    """Simulated Bitfinex exchange that processes orders with realistic behavior."""
    ...
    async def _execute_fill(self, order: SimulatedOrder, fill_price: float):
        ...
        try:
            ...
            for i in range(chunks):
                ...
                # Bitfinex: medium-slow fills (0.5-2.5s)
                ...
                # Bitfinex: wider price variation (+-0.025%)
                chunk_price = fill_price * (1 + random.uniform(-0.00025, 0.00025))
