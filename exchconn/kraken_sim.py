"""
Simulated Kraken Exchange Adapter.

Accepts orders, generates execution reports with realistic delays and partial fills.
Order IDs are prefixed with "KRK-".
Kraken characteristics: tight spreads (0.08% jitter), fast fills (0.3-1.5s delay),
small price variation (+-0.015%).
"""
...
# Kraken has tight spreads
PRICE_JITTER_PCT = 0.0008  # 0.08%
...
class KrakenSimulator:
    """Simulated Kraken exchange that processes orders with realistic behavior."""
    ...
    async def _execute_fill(self, order: SimulatedOrder, fill_price: float):
        ...
        try:
            ...
            for i in range(chunks):
                ...
                # Kraken: tight price variation (+-0.015%)
                chunk_price = fill_price * (1 + random.uniform(-0.00015, 0.00015))
