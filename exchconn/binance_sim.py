...
# Price jitter as a fraction of base price
PRICE_JITTER_PCT = 0.001  # 0.1%
...
class BinanceSimulator:
    """Simulated Binance exchange that processes orders with realistic behavior."""
        ...
        while self._running:
            try:
                ...
                for symbol in self._current_prices:
                    ...
                    # Keep price within reasonable bounds (0.5x - 1.5x of base)
    ...
    async def submit_order(self, fix_msg: FIXMessage):
        ...
        # For market orders, schedule fills immediately
    ...
    async def _execute_fill(self, order: SimulatedOrder, fill_price: float):
        ...
        try:
            ...
            for i in range(chunks):
                ...
                # Calculate fill quantity for this chunk
                if i == chunks - 1:
                    # Last chunk: fill the rest
                ...
                # Add small price variation per fill
    ...
    async def cancel_order(self, fix_msg: FIXMessage):
        ...
        if not order_id or order_id not in self._orders:
            # Reject the cancel - unknown order
            reject = execution_report(
                cl_ord_id=cl_ord_id,
        ...
        # Cancel the fill task if running
        if order_id in self._fill_tasks:
