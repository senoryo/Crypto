"""
Simulated Bybit Exchange Adapter.

Accepts orders, generates execution reports with realistic delays and partial fills.
Order IDs are prefixed with "BYB-".
Bybit characteristics: derivatives-focused, moderate spreads (0.10% jitter),
fast fills (0.3-1.5s delay), moderate price variation (+-0.02%).
"""
...
# Bybit has moderate spreads
PRICE_JITTER_PCT = 0.0010  # 0.10%
...
class BybitSimulator:
    """Simulated Bybit exchange that processes orders with realistic behavior."""
    ...
    async def _execute_fill(self, order: SimulatedOrder, fill_price: float):
        ...
        try:
            ...
            for i in range(chunks):
                ...
                # Bybit: moderate price variation (+-0.02%)
