"""Simulated Coinbase market data feed."""
...
# Base prices for each symbol (shared baseline)
...
class CoinbaseFeed:
    ...
    Generates realistic price ticks using a random walk model.
    Prices are slightly offset from Binance to simulate cross-exchange
    price differences (within 0.02% of the base).
    """
    ...
    def __init__(self):
        ...
        # Initialize prices with a Coinbase-specific offset (different from Binance)
        for symbol, base in BASE_PRICES.items():
            offset = base * random.uniform(-0.0002, 0.0002)
            self._prices[symbol] = base + offset
            self._open_prices[symbol] = self._prices[symbol]
            self._volumes[symbol] = random.uniform(800, 4000)
    ...
    async def _generate_ticks(self, symbol: str):
        ...
        while self._running:
            try:
                # Coinbase tends to have slightly different update intervals
                ...
                # Determine move magnitude
                if random.random() < 0.04:
                    # Large move (4% chance): 0.5-1.0% of price
                ...
                # Coinbase typically has slightly wider spreads than Binance
                spread = new_price * 0.00012
                bid = round(new_price - spread, precision)
                ask = round(new_price + spread, precision)

                # Volume per tick
                tick_volume = round(random.uniform(0.1, 8.0), 4)
                ...
                market_data = {
                    ...
                    "ask": ask,
                    "last": new_price,
                    "bid_size": round(random.uniform(0.3, 12.0), 4),
                    "ask_size": round(random.uniform(0.3, 12.0), 4),
